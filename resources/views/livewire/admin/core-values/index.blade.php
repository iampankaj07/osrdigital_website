                    ...
                    <tbody>
                        @forelse($coreValues as $coreValue)
                            <tr class="border-bottom">
                                <td class="py-3 px-3">
                                    <span class="badge badge-light text-dark border small">{{ $coreValue->sort_order }}</span>
                                </td>
                                <td class="py-3 px-3">
                                    <div class="d-flex align-items-start">
                                        @if($coreValue->icon)
                                            <i class="{{ $coreValue->icon }} text-primary mr-2 mt-1"></i>
                                        @endif
                                        <div class="font-weight-medium text-dark">{{ Str::limit($coreValue->title, 40) }}</div>
                                    </div>
                                </td>
                                <td class="py-3 px-3">
                                    <div class="text-muted small">{{ Str::limit($coreValue->description, 80) }}</div>
                                </td>
                                <td class="py-3 px-3 text-center">
                                    <span class="badge badge-{{ $coreValue->is_active ? 'success' : 'light' }} badge-sm">
                                ...
                                <td class="py-3 px-3 text-center">
                                    <div class="btn-group btn-group-sm" role="group">
                                        ...
                                        <button wire:click="toggleActive({{ $coreValue->id }})" 
                                                class="btn btn-outline-{{ $coreValue->is_active ? 'warning' : 'success' }} btn-sm border-0" 
                                                title="{{ $coreValue->is_active ? 'Deactivate' : 'Activate' }}">
                                            <i class="fas fa-{{ $coreValue->is_active ? 'pause' : 'play' }}"></i>
                                        </button>
                            ...
                            <tr>
                                <td colspan="5" class="text-center py-4">
                                    <div class="text-muted">
                                        <i class="fas fa-heart fa-lg mb-2 opacity-50"></i>
                                        <p class="mb-1 small">No core values found</p>
                                        <small class="text-muted">Click "Add Core Value" to create your first one</small>
                                    </div>
                                </td>
                            </tr>
                        @endforelse
            ...
            @if($coreValues->hasPages())
                <div class="d-flex justify-content-between align-items-center px-3 py-2 border-top bg-light">
                    <div class="text-muted small">
                        {{ $coreValues->firstItem() }}-{{ $coreValues->lastItem() }} of {{ $coreValues->total() }}
                    </div>
                    <div>
                        {{ $coreValues->links() }}
                    </div>
