<div>
    ...
    <div class="d-flex justify-content-between align-items-center mb-4">
        <div>
            <h1 class="h4 mb-1 font-weight-normal">Core Values</h1>
            <p class="text-muted small mb-0">Manage core values and principles</p>
        </div>
                ...
                <tbody class="bg-white divide-y divide-gray-100">
                    @forelse($coreValues as $coreValue)
                        <tr class="hover:bg-gray-50/50 transition-colors duration-150 group">
                            ...
                            <td class="px-6 py-4">
                                <div class="text-sm font-medium text-gray-900">{{ $coreValue->title }}</div>
                            </td>
                            <td class="px-6 py-4 whitespace-nowrap">
                                @if($coreValue->icon)
                                    <div class="w-8 h-8 bg-brand-orange-100 rounded-lg flex items-center justify-center">
                                        <i class="{{ $coreValue->icon }} text-brand-orange-600 text-sm"></i>
                                    </div>
                                @else
                            ...
                            <td class="px-6 py-4">
                                <div class="text-sm text-gray-600 max-w-xs">{{ Str::limit($coreValue->description, 50) }}</div>
                            </td>
                            <td class="px-6 py-4 whitespace-nowrap text-center">
                                <span class="inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium {{ $coreValue->is_active ? 'bg-green-100 text-green-800' : 'bg-gray-100 text-gray-800' }}">
                                    {{ $coreValue->is_active ? 'Active' : 'Inactive' }}
                                </span>
                            </td>
                            <td class="px-6 py-4 whitespace-nowrap text-center">
                                <div class="flex items-center justify-center space-x-1">
                                    <button wire:click="edit({{ $coreValue->id }})"
                                    ...
                                    <button wire:click="toggleActive({{ $coreValue->id }})"
                                            class="inline-flex items-center p-2 {{ $coreValue->is_active ? 'text-yellow-500 hover:text-yellow-600' : 'text-green-500 hover:text-green-600' }} hover:bg-gray-100 rounded-lg transition-colors duration-150"
                                            title="{{ $coreValue->is_active ? 'Deactivate' : 'Activate' }}">
                                        <i class="fas fa-{{ $coreValue->is_active ? 'pause' : 'play' }} text-sm"></i>
                                    </button>
                                    <button wire:click="delete({{ $coreValue->id }})"
                                            ...
                                            title="Delete"
                                            onclick="return confirm('Are you sure you want to delete this core value?')">
                        ...
                        <tr>
                            <td colspan="6" class="px-6 py-12 text-center">
                                <div class="flex flex-col items-center">
                                    ...
                                    <h3 class="text-sm font-medium text-gray-900 mb-1">No core values found</h3>
                                    <p class="text-sm text-gray-500">Create your first core value to get started.</p>
                                </div>
                            </td>
                        </tr>
                    @endforelse
    ...
    @if($showSlidePanel)
        ...
        <div class="slide-panel {{ $isClosing ? 'slide-out-right' : '' }}"
            ...
            <div class="slide-panel-header">
                <h5 class="mb-0">
                    @if($isCreating)
                        <i class="fas fa-plus mr-2"></i>Create New Core Value
                    @else
                        <i class="fas fa-edit mr-2"></i>Edit Core Value
                    @endif
                </h5>
            ...
            <div class="slide-panel-body">
                <form wire:submit="{{ $isCreating ? 'store' : 'update' }}">
                    <div class="form-group mb-3">
                        ...
                        <input type="text" wire:model="form.title" class="form-control @error('form.title') is-invalid @enderror" placeholder="Enter core value title">
                    ...
                    <div class="form-group mb-3">
                        ...
                        <textarea wire:model="form.description" class="form-control @error('form.description') is-invalid @enderror" rows="4" placeholder="Enter core value description"></textarea>
                    ...
                    <div class="d-flex justify-content-between pt-3 border-top mt-4">
                        ...
                        <button type="submit" class="btn btn-success">
                            <i class="fas fa-save me-2"></i>{{ $isCreating ? 'Create' : 'Update' }} Core Value
                        </button>
                    </div>
                </form>
            </div>
