                    ...
                    <tbody>
                        @forelse($teamValues as $value)
                            <tr class="border-bottom">
                                <td class="py-3 px-3">
                                    <span class="badge badge-light text-dark border small">{{ $value->sort_order }}</span>
                                </td>
                                <td class="py-3 px-3">
                                    <div class="d-flex align-items-start">
                                        @if($value->icon)
                                            <i class="{{ $value->icon }} text-info mr-2 mt-1"></i>
                                        @endif
                                        <div class="font-weight-medium text-dark">{{ Str::limit($value->title, 40) }}</div>
                                    </div>
                                </td>
                                <td class="py-3 px-3">
                                    <div class="text-muted small">{{ Str::limit($value->description, 80) }}</div>
                                </td>
                                <td class="py-3 px-3 text-center">
                                    <span class="badge badge-{{ $value->is_active ? 'success' : 'light' }} badge-sm">
                                ...
                                <td class="py-3 px-3 text-center">
                                    <div class="btn-group btn-group-sm" role="group">
                                        ...
                                        <button wire:click="toggleActive({{ $value->id }})" 
                                                class="btn btn-outline-{{ $value->is_active ? 'warning' : 'success' }} btn-sm border-0" 
                                                title="{{ $value->is_active ? 'Deactivate' : 'Activate' }}">
                                            <i class="fas fa-{{ $value->is_active ? 'pause' : 'play' }}"></i>
                                        </button>
                            ...
                            <tr>
                                <td colspan="5" class="text-center py-4">
                                    <div class="text-muted">
                                        ...
                                        <p class="mb-1 small">No team values found</p>
                                        <small class="text-muted">Click "Add Team Value" to create your first one</small>
                                    </div>
                                </td>
                            </tr>
                        @endforelse
            ...
            @if($teamValues->hasPages())
                <div class="d-flex justify-content-between align-items-center px-3 py-2 border-top bg-light">
                    <div class="text-muted small">
                        {{ $teamValues->firstItem() }}-{{ $teamValues->lastItem() }} of {{ $teamValues->total() }}
                    </div>
                    <div>
                        {{ $teamValues->links() }}
                    </div>
