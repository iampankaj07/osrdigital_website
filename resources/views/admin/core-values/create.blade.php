...
<div class="space-y-6">
    ...
    <div class="mb-8">
        <div class="flex items-center">
            ...
            <div>
                <h1 class="text-3xl font-bold text-gray-900">Create New Core Value</h1>
                <p class="text-gray-600 mt-2">Add a new core value to showcase your company principles</p>
            </div>
        </div>
    </div>
    ...
    <form method="POST" action="{{ route('admin.core-values.store') }}" id="coreValueForm">
        @csrf
            ...
            <div>
                ...
                <div class="flex items-center space-x-4">
                    <select name="icon" id="icon" required class="flex-1 px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-purple-500 focus:border-transparent @error('icon') border-red-500 @enderror">
                        ...
                        <option value="fas fa-heart" {{ old('icon') == 'fas fa-heart' ? 'selected' : '' }}>Heart (fas fa-heart)</option>
                        ...
                        <option value="fas fa-shield-alt" {{ old('icon') == 'fas fa-shield-alt' ? 'selected' : '' }}>Shield (fas fa-shield-alt)</option>
                        ...
                        <option value="fas fa-cog" {{ old('icon') == 'fas fa-cog' ? 'selected' : '' }}>Cog (fas fa-cog)</option>
        ...
        <div class="mb-8">
            ...
            <div>
                ...
                <textarea name="description" rows="4" required class="w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-purple-500 focus:border-transparent @error('description') border-red-500 @enderror" placeholder="Describe this core value...">{{ old('description') }}</textarea>
                @error('description')
                    <p class="text-red-500 text-sm mt-1">{{ $message }}</p>
                @enderror
            </div>
