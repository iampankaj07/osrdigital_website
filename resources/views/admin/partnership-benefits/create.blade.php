...
@section('title', 'Create Partnership Benefit')
...
<div class="space-y-6">
    ...
    <div class="mb-8">
        <div class="flex items-center">
            ...
            <div>
                <h1 class="text-3xl font-bold text-gray-900">Create New Partnership Benefit</h1>
                <p class="text-gray-600 mt-2">Add a new partnership benefit to showcase advantages</p>
            </div>
        </div>
    </div>
    ...
    <form method="POST" action="{{ route('admin.partnership-benefits.store') }}" id="benefitForm">
        @csrf
        ...
        <div class="mb-8">
            ...
            <div class="grid grid-cols-1 lg:grid-cols-2 gap-6">
                <div>
                    ...
                    <input type="text" name="title" value="{{ old('title') }}" required class="w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-purple-500 focus:border-transparent @error('title') border-red-500 @enderror" placeholder="e.g., Global Reach">
                    @error('title')
                ...
                <div class="flex items-center space-x-4">
                    <div class="flex-1">
                        ...
                        <select name="icon" id="icon" required class="w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-purple-500 focus:border-transparent @error('icon') border-red-500 @enderror" size="10">
                            ...
                            <option value="fas fa-bullhorn" {{ old('icon') == 'fas fa-bullhorn' ? 'selected' : '' }}>📢 Bullhorn (fas fa-bullhorn)</option>
                            <option value="fas fa-chart-bar" {{ old('icon') == 'fas fa-chart-bar' ? 'selected' : '' }}>📊 Chart Bar (fas fa-chart-bar)</option>
                            ...
                            <option value="fas fa-plus-circle" {{ old('icon') == 'fas fa-plus-circle' ? 'selected' : '' }}>➕ Plus Circle (fas fa-plus-circle)</option>
                        </select>
                    </div>
                    <div class="text-3xl" style="color: #EC681D" id="icon_preview">
                        <i class="{{ old('icon', 'fas fa-globe') }}"></i>
                    </div>
                </div>
                @error('icon')
                    <p class="text-red-500 text-sm mt-1">{{ $message }}</p>
        ...
        <div class="mb-8">
            ...
            <div>
                ...
                <textarea name="description" rows="4" required class="w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-purple-500 focus:border-transparent @error('description') border-red-500 @enderror" placeholder="Describe this partnership benefit...">{{ old('description') }}</textarea>
                @error('description')
                    <p class="text-red-500 text-sm mt-1">{{ $message }}</p>
                @enderror
            </div>
