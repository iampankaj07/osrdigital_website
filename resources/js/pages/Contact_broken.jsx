...
import { faEnvelope, faPhone, faMapMarkerAlt, faUser, faMessage, faPaperPlane, faCheckCircle, faExclamationCircle } from '@fortawesome/free-solid-svg-icons';
...
function Contact() {
    ...
    return (
        <div className={`min-h-screen transition-colors duration-300 ${isDark ? 'bg-gray-900' : 'bg-white'}`}>
            ...
            <section className={`py-16 md:py-20 lg:py-24 ${isDark ? 'bg-gray-900' : 'bg-white'}`}>
                <div className="container-minimal">
                    <div className="text-center max-w-3xl mx-auto">
                        <div className={`transition-all duration-1000 ${isVisible ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-10'}`}>
                            ...
                            {/* Main Content */}
                            <div className="mb-8">
                                <h1 className={`text-3xl md:text-4xl lg:text-5xl font-bold mb-4 leading-tight text-minimal-bold ${
                                    isDark ? 'text-white' : 'text-gray-900'
                                }`}>
                                    {contactData?.hero?.title || 'Contact Us'}
                                </h1>
                                <p className={`text-lg md:text-xl mb-6 leading-relaxed text-minimal ${
                                    ...
                                    {contactData ? (
                                        <>
                                            <div className="flex items-center">
                                                <FontAwesomeIcon icon={faEnvelope} className={`text-lg mr-3 ${isDark ? 'text-brand-orange-400' : 'text-brand-orange-600'}`} />
                                                <a href={`mailto:${contactData.contact?.email || 'hello@osrdigital.com'}`} className={`hover:text-brand-orange-500 ${isDark ? 'text-gray-300' : 'text-gray-700'}`}>
                                                    {contactData.contact?.email || 'hello@osrdigital.com'}
                                                </a>
                                            </div>
                                            <div className="flex items-center">
                                                <FontAwesomeIcon icon={faPhone} className={`text-lg mr-3 ${isDark ? 'text-brand-orange-400' : 'text-brand-orange-600'}`} />
                                                <a href={`tel:${contactData.contact?.phone || '+15551234567'}`} className={`hover:text-brand-orange-500 ${isDark ? 'text-gray-300' : 'text-gray-700'}`}>
                                                    {contactData.contact?.phone || '+1 (555) 123-4567'}
                                                </a>
                                            </div>
                                            <div className="flex items-start">
                                                <FontAwesomeIcon icon={faMapMarkerAlt} className={`text-lg mr-3 mt-1 ${isDark ? 'text-brand-orange-400' : 'text-brand-orange-600'}`} />
                                                <span className={`${isDark ? 'text-gray-300' : 'text-gray-700'}`}>
                                                    {contactData.contact?.address || 'Los Angeles, CA'}
                                                </span>
                                            </div>
                                        </>
                                    ) : (
